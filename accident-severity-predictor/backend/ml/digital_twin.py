...
class DigitalTwin:
    ...
    def __init__(self, city_key: str = "delhi", predictor=None):
        ...
        self.accident_mapper: Optional[AccidentSegmentMapper] = None
    ...
    def build_twin(self, force_rebuild: bool = False) -> dict:
        ...
        try:
            ...
            # Step 2: Map accidents
            logger.info("\n[2/4] Mapping accidents to segments...")
    ...
    def _build_accident_mapping(self, force_rebuild: bool):
        """Step 2: Map accidents to segments."""
        self.accident_mapper = AccidentSegmentMapper(
            self.edges_gdf, self.city_key
        )

        if force_rebuild or not self.accident_mapper.is_mapping_valid():
            self.segment_mapping = self.accident_mapper.map_all_accidents()
            self.accident_mapper.save_mapping(self.segment_mapping)
        else:
            self.segment_mapping = self.accident_mapper.load_mapping()
    ...
    def _initialize_simulator(self):
        ...
        # Always initialize - ScenarioSimulator handles
        # no-predictor case with rule-based risk scoring
        self.scenario_simulator = ScenarioSimulator(
            self.predictor,       # Can be None - handled internally
            self.segment_risks,
            self.city_key
        )
        if self.predictor is not None:
            ...
            logger.info(
                "Scenario simulator initialized with rule-based scoring"
            )
    ...
    def load_twin(self) -> dict:
        ...
        Faster alternative to build_twin() if cache exists.
        ...
        try:
            ...
            if not self.accident_mapper.is_mapping_valid():
                raise FileNotFoundError(
    ...
    def initialize_twin(self, force_rebuild: bool = False) -> dict:
        ...
        Main entry point - automatically detects cache.
    ...
    def get_segment_info(self, segment_id: str) -> dict:
        ...
        Returns:
            Segment info dict including risk, accidents, etc.
        """
    ...
    def get_top_dangerous_segments(self, n: int = 10,
        ...
        if self.risk_calculator:
            return self.risk_calculator.get_top_dangerous_segments(
                n, min_risk
            )
    ...
    def get_heatmap_data(self, heatmap_type: str = "grid") -> dict:
        ...
        if heatmap_type == "grid":
            ...
            raise ValueError(
                ...
                f"Use 'grid' or 'segments'"
            )

    def simulate_scenario(self, segment_id: str, scenario_type: str,
                           **kwargs) -> dict:
        """
        Run scenario simulation on a segment.
        """
        # Initialize simulator on-demand if not already done
    ...
    def get_stats(self) -> dict:
        ...
        if self.accident_mapper:
            stats["mapping_stats"] = self.accident_mapper.get_stats()
    ...
    def _generate_top_dangerous(self):
        ...
        logger.info(
            f"Top dangerous segments saved: {self.top_dangerous_path}"
        )
    ...
    def refresh_twin(self) -> dict:
        ...
        Alias for build_twin(force_rebuild=True)
