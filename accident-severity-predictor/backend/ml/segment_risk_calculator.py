...
class SegmentRiskCalculator:
    ...
    def calculate_historical_risk(self, segment_data: dict) -> float:
        """
        Calculate risk score based on accident history.

        Formula:
        - Accident density (accidents per km per year)
        - Weighted by severity (Fatal=4, Grievous=3, Minor=2, NoInjury=1)
        ...
        length_km = max(length_m / 1000, 0.01)  # Avoid division by zero
        ...
        # Assume data covers 3 years (adjust based on actual data)
        years_of_data = 3.0
        ...
        # Empirical scaling: density > 20 → risk = 100
        risk_score = min(100, (density / 20) * 100)
